...
public class Pathfinding : MonoBehaviour
{
    ...
    public List<Vector3Int> Launch(Vector2Int startPosition, Vector2Int endPosition, int[,] grid)
    {
        var path = FindPath(startPosition, endPosition, grid);
    ...
    List<Node> FindPath(Vector2Int start, Vector2Int end, int[,] grid)
    {
        ...
        while (openList.Count > 0)
        {
            ...
            foreach (var neighbor in GetNeighbors(nodes, current, grid))
            {
    ...
    List<Node> GetNeighbors(Node[,] nodes, Node node, int[,] grid)
    {
